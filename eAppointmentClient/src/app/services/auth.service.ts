import { Injectable } from '@angular/core';
import { Router } from '@angular/router';
import { TokenModel } from '../models/token.model';
import { jwtDecode, JwtPayload } from 'jwt-decode';  //Tokeni parçalayıp içerisinden kullanıcı verilerini çekmek için kurduk

@Injectable({
  providedIn: 'root'
})
export class AuthService {

  tokenDecode: TokenModel = new TokenModel();

  constructor(
    private router:Router
  ) { }

  isAuthenticated(){
    const token: string | null = localStorage.getItem("token");
    if(token){
      const decode: JwtPayload | any = jwtDecode(token);
     
      const exp = decode.exp; //expire time
      const now = new Date().getTime() /1000;
      if(now>exp){  //tokenin expire süresi dolmuşsa tokeni sil, logine yönlendir ve authenticated false dön

        localStorage.removeItem("token");  
        this.router.navigateByUrl("/login");
        return false;
      }
      //Token parçalanıyor
      this.tokenDecode.id = decode["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/nameidentifier"];
      this.tokenDecode.name = decode["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name"];
      this.tokenDecode.email = decode["http://schemas.xmlsoap.org/ws/2005/05/identity/claims/emailaddress"];
      this.tokenDecode.userName = decode["UserName"];
      this.tokenDecode.roles = JSON.parse(decode["http://schemas.microsoft.com/ws/2008/06/identity/claims/role"]); //rolleri listeye çevirdim
      return true;
    }
    this.router.navigateByUrl("/login");
    return false;
  }
}
